...
(function($) {
	...
	let GetDisplayFilesize = function(numbytes, adjustprecision, units) {
		if (numbytes == 0)  return '0 Bytes';
		if (numbytes == 1)  return '1 Byte';

		numbytes = Math.abs(numbytes);
		let magnitude, abbreviations;
		...
		let pos = Math.floor(Math.log(numbytes) / Math.log(magnitude));
		let result = (numbytes / Math.pow(magnitude, pos));

		return (pos == 0 || (adjustprecision && result >= 99.995) ? result.toFixed(0) : result.toFixed(2)) + ' ' + abbreviations[pos];
	};

	let DisplayPreviewDialog = function(preview, endelem, inforow, data, settings) {
		let previewbackground = $('<div>').addClass('ff_fileupload_dialog_background');
		let previewclone = preview.clone(true, true).click(function(e) {
			e.stopPropagation();
		});
		let previewdialog = $('<div>').addClass('ff_fileupload_dialog_main').append(previewclone);

		let HidePreviewDialog = function() {
	...
	let InitShowAriaLabelInfo = function(inforow) {
	...
	$.fn.FancyFileUpload = function(options) {
		this.each(function() {
			...
			if ($this.data('fancy-fileupload') && typeof($this.data('fancy-fileupload')) === 'object')
			{
				...
				let data = $this.data('fancy-fileupload');
		...
		let settings = $.extend({}, $.fn.FancyFileUpload.defaults, options);
		...
		$(window).on('beforeunload.fancy_fileupload', function(e) {
			let active = $('.ff_fileupload_uploading, .ff_fileupload_starting');
			let queued = $('.ff_fileupload_queued');
		...
		let AddFile = function(uploads, e, data) {
			...
			let StartUpload = function(e) {
				e.preventDefault();

				// Set filename.
				if (settings.edit && !data.ff_info.errors.length)
				{
					let fileinput = inforow.find('.ff_fileupload_filename input');
					if (fileinput.length)
					{
						let newfilename = fileinput.val();
			...
			{
				let found = false;
				for (let x = 0; x < settings.accept.length && !found; x++)
				{
		...
		let UploadProgress = function(e, data) {
			let progress = (data.total < 1 ? 0 : data.loaded / data.total * 100);
		...
		let UploadFailed = function(e, data) {
		...
		let UploadDone = function(e, data) {
		...
		let UploadChunkSend = function(e, data) {
			if (data.ff_info)
			{
		...
		let UploadChunkDone = function(e, data) {
		...
		return this.each(function() {
			...
			let form = $('<form>').addClass('ff_fileupload_hidden').attr({
				'action' : settings.url,
				'method' : 'post',
			...
			let fileinputname = $this.attr('name');
			...
			if ($this.attr('accept'))
			{
				...
				if (!settings.accept)
				{
					...
					settings.accept = [];
					for (let x = 0; x < accept.length; x++)
					{
						let opt = $.trim(accept[x]).toLowerCase();
			...
			let fileuploadwrap = $('<div>').addClass('ff_fileupload_wrap');
			...
			let dropzonewrap = $('<div>').addClass('ff_fileupload_dropzone_wrap');
			let dropzone = $('<button>').addClass('ff_fileupload_dropzone').attr('type', 'button').attr('aria-label', Translate('Browse, drag-and-drop, or paste files to upload'));
			...
			let dropzonetools = $('<div>').addClass('ff_fileupload_dropzone_tools');
			dropzonewrap.append(dropzonetools);

			// Record audio.
			if (settings.recordaudio && navigator.mediaDevices && window.MediaRecorder)
			{
				let audiobutton = $('<button>').addClass('ff_fileupload_dropzone_tool').addClass('ff_fileupload_recordaudio').attr('type', 'button').attr('aria-label', Translate('Record audio using a microphone'));
			...
			{
				let videobutton = $('<button>').addClass('ff_fileupload_dropzone_tool').addClass('ff_fileupload_recordvideo').attr('type', 'button').attr('aria-label', Translate('Record video using a camera'));
				dropzonetools.append(videobutton);

				let videorecpreview = $('<video>').prop('muted', false).prop('autoplay', true).addClass('ff_fileupload_recordvideo_preview').addClass('ff_fileupload_hidden');
			...
			let uploads = $('<table>').addClass('ff_fileupload_uploads');
